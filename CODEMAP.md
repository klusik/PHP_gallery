...
| Create gallery | `app/controllers/admin_galleries_discovery.php`, `app/views/admin_gallery_forms.php`, `app/services/gallery_sidecars.php`, `app/controllers/public_gallery_cards.php` | Public-page `+` opens a title-only panel with parent context and title completion. The canonical create response opens the new gallery editor in place. The direct Admin page retains extended creation fields and the upload branch remains separate. |
| Per-admin creation defaults | `app/models/gallery_creation_preferences.php`, `app/services/gallery_creation_preferences.php`, `database/migrations/202609250001_gallery_creation_preferences.php`, `app/controllers/admin_gallery_form_models.php` | One user-owned row stores optional SimBrief Pilot ID/name and source-language defaults. Structured schema inspection permits empty read defaults when unavailable, but explicit Remember writes require verified storage. |
| Compact gallery editor | `app/controllers/admin_galleries_edit_page/`, `app/views/admin_gallery_edit_tabs.php`, `public/assets/gallery-modules/admin-gallery-compact-editor.js`, `public/assets/gallery-modules/admin-tabs.js`, `public/assets/styles/side-panel.css` | Identity, API, Access and Display prioritize frequent controls; infrequent controls use Advanced disclosures and contextual help. Drawer tabs resolve only their own injected panels, and the Save gallery bar remains visible during scrolling. |
