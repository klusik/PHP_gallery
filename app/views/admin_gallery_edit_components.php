 ...
 *
 * File: app/views/admin_gallery_edit_components.php
 * Module Type: View
 *
 * Purpose:
 *   Renders remaining Gallery Editor presentation components from controller-prepared state.
 *
 * Responsibilities:
 *   - Render AI metadata regeneration controls
 *   - Render image bulk/move toolbar
 *   - Render gallery branding asset controls
 *   - Render the Admin image reorder browser controller script
 *
 * Author:
 *   Rudolf Klusal
 *
 ...
 *   - Feature/schema policy, gallery lookup, branding URL resolution, destination picker generation,
 *     URL generation, and CSRF issuance remain outside this view.
 *
 * Last Updated:
 *   2026-09-13
 */
...
function view_render_admin_gallery_ai_reprocess_panel(array $viewModel): void
{
    ...
    $confirmMessage = (string) ($viewModel['confirm_message'] ?? '');
...
function view_render_admin_image_bulk_toolbar(array $viewModel): void
{
    ...
    foreach ([['action','step_choose_action','step_choose_action_help'],['target','step_target','step_target_help'],['confirm','step_confirm','step_confirm_help'],['complete','step_complete','step_complete_help']] as $index => $step) {
        echo '<div class="admin-image-move-step' . ($index === 0 ? ' is-active' : '') . '" data-admin-image-move-step="' . e($step[0]) . '"><span>' . ($index + 1) . '</span><div><strong>' . e(t('admin.gallery_editor.' . $step[1])) . '</strong><p>' . e(t('admin.gallery_editor.' . $step[2])) . '</p></div></div>';
    }
