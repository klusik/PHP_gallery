...
function admin_gallery_report_nearest_known_place(float $lat, float $lng): array
{
    ...
    if ($nearest !== null) {
        return [
            'label' => 'Closest known area: ' . (string) $nearest['label'],
            'nearest_reference' => (string) $nearest['name'],
            'place_kind' => (string) $nearest['kind'],
            'place_match' => 'nearest fallback',
            'distance_km' => round((float) $nearest['distance_km'], 1),
        ];
    }

    return [
        'label' => 'Area around ' . number_format($lat, 3, '.', '') . ', ' . number_format($lng, 3, '.', ''),
        'nearest_reference' => '',
        'place_kind' => 'coordinate fallback',
        'place_match' => 'coordinate fallback',
        'distance_km' => null,
    ];
}
...
{
    return [
        ['name' => 'Friedrichshafen', 'label' => 'Friedrichshafen / Bodensee area', 'lat' => 47.6505, 'lng' => 9.4790, 'radius_km' => 45.0, 'kind' => 'regional area', 'priority' => 4.0],
        ['name' => 'Konstanz', 'label' => 'Konstanz / Bodensee area', 'lat' => 47.6779, 'lng' => 9.1732, 'radius_km' => 38.0, 'kind' => 'regional area', 'priority' => 3.0],
        ['name' => 'Lindau', 'label' => 'Lindau / Bodensee area', 'lat' => 47.5460, 'lng' => 9.6830, 'radius_km' => 24.0, 'kind' => 'regional area', 'priority' => 2.0],
        ['name' => 'Ravensburg', 'label' => 'Ravensburg / Upper Swabia area', 'lat' => 47.7819, 'lng' => 9.6106, 'radius_km' => 28.0, 'kind' => 'regional area'],
        ...
        ['name' => 'Plasy', 'label' => 'Plasy / LKPS area', 'lat' => 49.9346, 'lng' => 13.3906, 'radius_km' => 16.0, 'kind' => 'local area', 'priority' => 5.0],
...
function admin_gallery_report_render_html(array $report): string
{
    ...
    $title = 'PHP Gallery complete overview report';
    ...
    $html .= '<header><p class="eyebrow">PHP Gallery Admin Report</p><h1>' . admin_gallery_report_h($title) . '</h1><p class="muted">Generated ' . admin_gallery_report_h((string) ($site['generated_at_utc'] ?? gmdate('c'))) . ' UTC. This is a single self-contained HTML file. It was generated in browser-driven batches and was not saved as a report file on the server.</p><div class="print-note">Use the browser print dialog to save this HTML as PDF when needed.</div></header>';

    $html .= '<section class="panel"><h2>Executive overview</h2><div class="grid">'
        . admin_gallery_report_metric_card('Version', (string) ($site['version'] ?? ''), (string) ($site['site_name'] ?? 'PHP Gallery'))
        . admin_gallery_report_metric_card('Galleries', admin_gallery_report_n($galleries['total'] ?? 0), admin_gallery_report_n($galleries['root_count'] ?? 0) . ' root, ' . admin_gallery_report_n($galleries['nested_count'] ?? 0) . ' nested')
        . admin_gallery_report_metric_card('Images', admin_gallery_report_n($images['image_count'] ?? 0), admin_gallery_report_n($images['public_count'] ?? 0) . ' public')
        . admin_gallery_report_metric_card('Source photos', admin_gallery_report_bytes($storage['original_bytes'] ?? 0), 'Average ' . admin_gallery_report_bytes($storage['average_original_bytes'] ?? 0))
        . admin_gallery_report_metric_card('Generated media', admin_gallery_report_bytes($storage['generated_bytes'] ?? 0), admin_gallery_report_n($storage['generated_thumbnail_count'] ?? 0) . ' thumbnails')
        . admin_gallery_report_metric_card('Database', admin_gallery_report_bytes((int) ($database['usage']['total_bytes'] ?? 0)), (string) ($database['database_name'] ?? ''))
        . admin_gallery_report_metric_card('EXIF date', admin_gallery_report_percent($images['exif_date_count'] ?? 0, $images['image_count'] ?? 0), admin_gallery_report_n($images['exif_date_count'] ?? 0) . ' images')
        . admin_gallery_report_metric_card('GPS', admin_gallery_report_percent($images['gps_count'] ?? 0, $images['image_count'] ?? 0), admin_gallery_report_n($images['gps_count'] ?? 0) . ' images')
        . admin_gallery_report_metric_card('Telemetry window', admin_gallery_report_n($telemetry['days'] ?? 0) . ' days', !empty($telemetry['available']) ? 'available' : 'not available')
        . '</div></section>';

    $html .= '<section class="panel"><h2>Generation and runtime</h2><div class="split"><div>'
        . admin_gallery_report_definition_list([
            'Report duration' => admin_gallery_report_duration((int) ($report['duration_seconds'] ?? 0)),
            'Images processed' => admin_gallery_report_n($report['processed'] ?? 0) . ' / ' . admin_gallery_report_n($report['total'] ?? 0),
            'PHP' => (string) ($runtime['php_version'] ?? ''),
            'SAPI' => (string) ($runtime['php_sapi'] ?? ''),
            'Server software' => (string) ($runtime['server_software'] ?? ''),
            'OS' => (string) ($runtime['os'] ?? ''),
            'MySQL/MariaDB' => (string) ($runtime['mysql_version'] ?? ''),
            'Memory limit' => (string) ($runtime['memory_limit'] ?? ''),
            'Peak memory used' => admin_gallery_report_bytes($runtime['peak_memory_usage_bytes'] ?? 0),
            'Server RAM' => admin_gallery_report_server_memory_label(is_array($runtime['server_memory'] ?? null) ? $runtime['server_memory'] : []),
            'Load average' => admin_gallery_report_load_average_label(is_array($runtime['load_average'] ?? null) ? $runtime['load_average'] : []),
            'GD' => admin_gallery_report_gd_label(is_array($runtime['gd_info'] ?? null) ? $runtime['gd_info'] : []),
            'Imagick' => (string) ($runtime['imagick_version'] ?? ''),
            'Max execution time' => (string) ($runtime['max_execution_time'] ?? '') . ' s',
            'Upload max filesize' => (string) ($runtime['upload_max_filesize'] ?? ''),
            'Post max size' => (string) ($runtime['post_max_size'] ?? ''),
            'Timezone' => (string) ($runtime['timezone'] ?? ''),
        ]) . '</div><div><h3>PHP extensions</h3>' . admin_gallery_report_table($runtime['extensions'] ?? [], [
            ['key' => 'extension', 'label' => 'Extension'],
            ['key' => 'loaded', 'label' => 'Loaded'],
        ]) . '</div></div></section>';

    $html .= '<section class="panel"><h2>Storage details</h2><div class="grid">'
        . admin_gallery_report_metric_card('Original sources', admin_gallery_report_bytes($storage['original_bytes'] ?? 0), admin_gallery_report_n($storage['known_source_size_count'] ?? 0) . ' known sizes')
        . admin_gallery_report_metric_card('Thumbnails', admin_gallery_report_bytes($storage['generated_thumbnail_bytes'] ?? 0), admin_gallery_report_n($storage['generated_thumbnail_count'] ?? 0) . ' files')
        . admin_gallery_report_metric_card('Display masters', admin_gallery_report_bytes($storage['display_master_bytes'] ?? 0), admin_gallery_report_n($storage['display_master_count'] ?? 0) . ' files')
        . admin_gallery_report_metric_card('Total picture storage', admin_gallery_report_bytes($storage['total_picture_bytes'] ?? 0), 'Generated/original ' . admin_gallery_report_n($storage['generated_to_original_percent'] ?? 0, 1) . ' %')
        . admin_gallery_report_metric_card('Largest source', admin_gallery_report_bytes($storage['largest_original_bytes'] ?? 0), (string) ($storage['largest_original_name'] ?? ''))
        . admin_gallery_report_metric_card('Scan errors', admin_gallery_report_n($storage['thumbnail_scan_errors'] ?? 0), !empty($storage['thumbnail_metadata_used']) ? 'thumbnail metadata used' : 'filesystem checks used')
        . '</div><div class="split"><div><h3>Source types</h3>' . admin_gallery_report_bar_table($storage['type_rows'] ?? [], 'count', 'bytes') . '</div><div><h3>Generated media types</h3>' . admin_gallery_report_bar_table($storage['generated_type_rows'] ?? [], 'count', 'bytes') . '</div></div><h3>Configured data paths</h3>' . admin_gallery_report_table($report['data_paths']['paths'] ?? [], [
            ['key' => 'label', 'label' => 'Path'],
            ['key' => 'path', 'label' => 'Filesystem value'],
            ['key' => 'exists', 'label' => 'Exists'],
            ['key' => 'readable', 'label' => 'Readable'],
            ['key' => 'writable', 'label' => 'Writable'],
            ['key' => 'free_bytes', 'label' => 'Free', 'format' => fn($v): string => admin_gallery_report_bytes($v)],
            ['key' => 'total_bytes', 'label' => 'Total', 'format' => fn($v): string => admin_gallery_report_bytes($v)],
        ]) . '</section>';

    $html .= '<section class="panel"><h2>Database overview</h2><div class="grid">'
        . admin_gallery_report_metric_card('Total DB size', admin_gallery_report_bytes($database['usage']['total_bytes'] ?? 0), 'data + indexes')
        . admin_gallery_report_metric_card('Gallery DB size', admin_gallery_report_bytes($database['usage']['gallery_bytes'] ?? 0), 'content-related tables')
        . admin_gallery_report_metric_card('Operational DB size', admin_gallery_report_bytes($database['usage']['operational_bytes'] ?? 0), 'logs, telemetry, settings, users')
        . admin_gallery_report_metric_card('Rows exact', !empty($database['usage']['table_rows_exact_available']) ? admin_gallery_report_n($database['usage']['table_rows_exact'] ?? 0) : admin_gallery_report_n($database['usage']['table_rows_estimate'] ?? 0), !empty($database['usage']['table_rows_exact_available']) ? 'COUNT(*) across DB tables' : 'engine estimate')
        . '</div><h3>Database tables</h3>' . admin_gallery_report_table($database['usage']['table_rows'] ?? [], [
            ['key' => 'table_name', 'label' => 'Table'],
            ['key' => 'rows_exact', 'label' => 'Rows exact', 'format' => fn($v): string => $v === null ? '' : admin_gallery_report_n($v)],
            ['key' => 'table_rows_estimate', 'label' => 'Rows estimate', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'data_bytes', 'label' => 'Data', 'format' => fn($v): string => admin_gallery_report_bytes($v)],
            ['key' => 'index_bytes', 'label' => 'Index', 'format' => fn($v): string => admin_gallery_report_bytes($v)],
            ['key' => 'total_bytes', 'label' => 'Total', 'format' => fn($v): string => admin_gallery_report_bytes($v)],
            ['key' => 'engine', 'label' => 'Engine'],
            ['key' => 'row_count_source', 'label' => 'Row source'],
        ]) . '<h3>Application table counts</h3>' . admin_gallery_report_table($report['tables'] ?? [], [
            ['key' => 'table_name', 'label' => 'Table'],
            ['key' => 'exists', 'label' => 'Exists'],
            ['key' => 'rows', 'label' => 'Rows', 'format' => fn($v): string => $v === null ? '' : admin_gallery_report_n($v)],
            ['key' => 'error', 'label' => 'Error'],
        ]) . '</section>';

    $html .= '<section class="panel"><h2>Gallery structure</h2><div class="split"><div><h3>Visibility</h3>' . admin_gallery_report_bar_table($galleries['visibility_rows'] ?? [], 'count') . '</div><div><h3>Access mode</h3>' . admin_gallery_report_bar_table($galleries['access_rows'] ?? [], 'count') . '</div></div><div class="grid">'
        . admin_gallery_report_metric_card('Empty galleries', admin_gallery_report_n($galleries['empty_count'] ?? 0), 'direct image count is zero')
        . admin_gallery_report_metric_card('Deepest nesting', admin_gallery_report_n($galleries['deepest_depth'] ?? 0), 'parent chain depth')
        . admin_gallery_report_metric_card('Dated galleries', admin_gallery_report_n($galleries['dated_gallery_count'] ?? 0), 'manual date range present')
        . '</div><h3>Largest galleries</h3>' . admin_gallery_report_table($galleries['largest_rows'] ?? [], [
            ['key' => 'id', 'label' => 'ID'],
            ['key' => 'title', 'label' => 'Title'],
            ['key' => 'folder_path', 'label' => 'Folder'],
            ['key' => 'visibility', 'label' => 'Visibility'],
            ['key' => 'image_count', 'label' => 'Images', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'source_bytes', 'label' => 'Source bytes', 'format' => fn($v): string => admin_gallery_report_bytes($v)],
        ]) . '<h3>Complete gallery list</h3>' . admin_gallery_report_table($report['gallery_rows'] ?? [], [
            ['key' => 'id', 'label' => 'ID'],
            ...
            ['key' => 'access_listing', 'label' => 'Listing'],
            ['key' => 'image_count', 'label' => 'Images', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'source_bytes', 'label' => 'Source', 'format' => fn($v): string => admin_gallery_report_bytes($v)],
            ['key' => 'gps_images', 'label' => 'GPS images', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'date_start', 'label' => 'Date from'],
            ['key' => 'date_end', 'label' => 'Date to'],
            ['key' => 'gps_map_enabled', 'label' => 'GPS map override'],
            ['key' => 'picture_game_enabled', 'label' => 'Game'],
            ['key' => 'updated_at', 'label' => 'Updated'],
        ]) . '</section>';

    $html .= '<section class="panel"><h2>Images and EXIF statistics</h2><div class="grid">'
        . admin_gallery_report_metric_card('Images with EXIF date', admin_gallery_report_n($images['exif_date_count'] ?? 0), admin_gallery_report_percent($images['exif_date_count'] ?? 0, $images['image_count'] ?? 0))
        . admin_gallery_report_metric_card('Images with GPS', admin_gallery_report_n($images['gps_count'] ?? 0), admin_gallery_report_percent($images['gps_count'] ?? 0, $images['image_count'] ?? 0))
        . admin_gallery_report_metric_card('Both date and GPS', admin_gallery_report_n($images['both_exif_date_and_gps_count'] ?? 0), admin_gallery_report_percent($images['both_exif_date_and_gps_count'] ?? 0, $images['image_count'] ?? 0))
        . admin_gallery_report_metric_card('Neither date nor GPS', admin_gallery_report_n($images['neither_exif_date_nor_gps_count'] ?? 0), admin_gallery_report_percent($images['neither_exif_date_nor_gps_count'] ?? 0, $images['image_count'] ?? 0))
        . admin_gallery_report_metric_card('Known camera', admin_gallery_report_n($images['camera_known_count'] ?? 0), admin_gallery_report_percent($images['camera_known_count'] ?? 0, $images['image_count'] ?? 0))
        . admin_gallery_report_metric_card('Known lens', admin_gallery_report_n($images['lens_known_count'] ?? 0), admin_gallery_report_percent($images['lens_known_count'] ?? 0, $images['image_count'] ?? 0))
        . admin_gallery_report_metric_card('Average megapixels', admin_gallery_report_n($images['average_megapixels'] ?? 0, 2) . ' MP', admin_gallery_report_n($images['known_dimensions_count'] ?? 0) . ' images with dimensions')
        . admin_gallery_report_metric_card('EXIF date range', admin_gallery_report_short_date((string) ($images['exif_date_min'] ?? '')) . ' to ' . admin_gallery_report_short_date((string) ($images['exif_date_max'] ?? '')), 'from imported metadata')
        . '</div><div class="split"><div><h3>Image types</h3>' . admin_gallery_report_bar_table($images['type_rows'] ?? [], 'count', 'bytes') . '</div><div><h3>Orientation</h3>' . admin_gallery_report_bar_table($images['dimension_rows'] ?? [], 'count') . '</div></div><div class="split"><div><h3>Top cameras</h3>' . admin_gallery_report_bar_table($images['camera_rows'] ?? [], 'count') . '</div><div><h3>Top lenses</h3>' . admin_gallery_report_bar_table($images['lens_rows'] ?? [], 'count') . '</div></div><div class="split"><div><h3>ISO buckets</h3>' . admin_gallery_report_bar_table($images['iso_rows'] ?? [], 'count') . '</div><div><h3>Top focal lengths</h3>' . admin_gallery_report_bar_table($images['focal_rows'] ?? [], 'count') . '</div></div></section>';

    $html .= '<section class="panel"><h2>GPS place overview</h2><p class="muted">GPS points are clustered into approximate ' . admin_gallery_report_n(ADMIN_GALLERY_REPORT_GPS_AREA_KM, 0) . ' km areas. Probable simulator or game captures are excluded from place clustering using filename, file type, and missing camera EXIF heuristics.</p><div class="grid">'
        . admin_gallery_report_metric_card('GPS points found', admin_gallery_report_n($images['gps_count'] ?? 0), 'all images with coordinates')
        . admin_gallery_report_metric_card('Clustered as real places', admin_gallery_report_n($images['clustered_gps_count'] ?? 0), 'used in place overview')
        . admin_gallery_report_metric_card('Excluded probable game GPS', admin_gallery_report_n($images['probable_game_gps_count'] ?? 0), 'not used for real-world place frequency')
        . admin_gallery_report_metric_card('Latitude range', admin_gallery_report_n($images['gps_lat_min'] ?? 0, 4) . ' to ' . admin_gallery_report_n($images['gps_lat_max'] ?? 0, 4), 'coordinate bounds')
        . admin_gallery_report_metric_card('Longitude range', admin_gallery_report_n($images['gps_lng_min'] ?? 0, 4) . ' to ' . admin_gallery_report_n($images['gps_lng_max'] ?? 0, 4), 'coordinate bounds')
        . '</div><h3>Frequent GPS areas</h3>' . admin_gallery_report_table($images['gps_cluster_rows'] ?? [], [
            ['key' => 'label', 'label' => 'Area'],
            ['key' => 'nearest_reference', 'label' => 'Reference'],
            ['key' => 'place_match', 'label' => 'Match'],
            ['key' => 'place_distance_km', 'label' => 'Distance', 'format' => fn($v): string => $v === null || $v === '' ? '' : admin_gallery_report_n($v, 1) . ' km'],
            ['key' => 'count', 'label' => 'Images', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'lat', 'label' => 'Center lat'],
            ['key' => 'lng', 'label' => 'Center lng'],
            ['key' => 'gallery_count', 'label' => 'Galleries', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'gallery_labels', 'label' => 'Example galleries', 'format' => fn($v): string => is_array($v) ? implode(', ', array_slice($v, 0, 6)) : (string) $v],
            ['key' => 'first_date', 'label' => 'First EXIF date', 'format' => fn($v): string => admin_gallery_report_short_date((string) $v)],
            ['key' => 'last_date', 'label' => 'Last EXIF date', 'format' => fn($v): string => admin_gallery_report_short_date((string) $v)],
            ['key' => 'sample_images', 'label' => 'Sample images', 'format' => fn($v): string => is_array($v) ? implode(', ', array_slice($v, 0, 4)) : (string) $v],
        ]) . '</section>';

    $html .= admin_gallery_report_render_telemetry_section($telemetry);
    $html .= admin_gallery_report_render_logs_section($logs);

    $html .= '<section class="panel"><h2>Tags, votes, and largest images</h2><div class="split"><div><h3>Gallery tags</h3>' . admin_gallery_report_bar_table($report['tags']['gallery_tag_rows'] ?? [], 'count') . '</div><div><h3>Image tags</h3>' . admin_gallery_report_bar_table($report['tags']['image_tag_rows'] ?? [], 'count') . '</div></div><h3>Largest source images</h3>' . admin_gallery_report_table($report['top_images'] ?? [], [
            ['key' => 'id', 'label' => 'ID'],
            ['key' => 'filename', 'label' => 'Filename'],
            ['key' => 'gallery_title', 'label' => 'Gallery'],
            ['key' => 'relative_path', 'label' => 'Relative path'],
            ['key' => 'mime_type', 'label' => 'MIME'],
            ['key' => 'file_size', 'label' => 'Size', 'format' => fn($v): string => admin_gallery_report_bytes($v)],
            ['key' => 'width', 'label' => 'Width'],
            ['key' => 'height', 'label' => 'Height'],
            ['key' => 'visibility', 'label' => 'Visibility'],
        ]) . '</section>';

    $html .= '<section class="panel"><h2>Feature and settings overview</h2><h3>Selected application settings</h3>' . admin_gallery_report_table($report['features']['settings_rows'] ?? [], [
            ['key' => 'setting_key', 'label' => 'Key'],
            ['key' => 'setting_value', 'label' => 'Value'],
            ['key' => 'updated_at', 'label' => 'Updated'],
        ]) . '</section>';

    $html .= '<footer class="panel"><h2>Report notes</h2><ul><li>GPS area labels use an expanded offline place list with city, town, and regional references. If a cluster is outside every configured place radius, the report still shows the nearest known reference and marks it as a nearest fallback.</li><li>Simulator and game GPS exclusion is heuristic. It avoids obvious PNG/WebP captures without camera EXIF and names containing simulator or screenshot markers.</li><li>Database sizes come from MySQL/MariaDB table metadata. Row counts use exact COUNT(*) values when the host allows them; engine estimates are shown separately.</li><li>No report file was written to the server. The browser received this HTML and can save it locally.</li></ul></footer>';
    $html .= '</main></body></html>';
    return $html;
}
...
function admin_gallery_report_render_telemetry_section(array $telemetry): string
{
    if (empty($telemetry['available'])) {
        return '<section class="panel"><h2>Telemetry</h2><p class="muted">Telemetry is not available on this installation or the schema is not migrated yet.</p></section>';
    }
    $session = is_array($telemetry['session_summary'] ?? null) ? $telemetry['session_summary'] : [];
    $html = '<section class="panel"><h2>Telemetry and usage</h2><p class="muted">Telemetry window: last ' . admin_gallery_report_h((string) ($telemetry['days'] ?? 0)) . ' days. Public telemetry is ' . (!empty($telemetry['public_enabled']) ? 'enabled' : 'disabled') . '.</p><div class="grid">'
        . admin_gallery_report_metric_card('Sessions', admin_gallery_report_n($session['sessions'] ?? 0), 'anonymous session hashes')
        . admin_gallery_report_metric_card('Page views', admin_gallery_report_n($session['page_views'] ?? 0), admin_gallery_report_n($session['avg_pages_per_session'] ?? 0, 2) . ' per session')
        . admin_gallery_report_metric_card('Photo views', admin_gallery_report_n($session['photo_views'] ?? 0), admin_gallery_report_n($session['avg_photos_per_session'] ?? 0, 2) . ' per session')
        . admin_gallery_report_metric_card('Duration', admin_gallery_report_duration((int) ($session['duration_seconds'] ?? 0)), 'capped total')
        . admin_gallery_report_metric_card('Bounced sessions', admin_gallery_report_n($session['bounced_sessions'] ?? 0), 'single-page sessions')
        . '</div><h3>Telemetry windows</h3>' . admin_gallery_report_table($telemetry['windows'] ?? [], [
            ['key' => 'days', 'label' => 'Days'],
            ['key' => 'sessions', 'label' => 'Sessions', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'page_views', 'label' => 'Page views', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'photo_views', 'label' => 'Photo views', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'duration_seconds', 'label' => 'Duration', 'format' => fn($v): string => admin_gallery_report_duration((int) $v)],
            ['key' => 'db_queries', 'label' => 'DB queries', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'db_slow', 'label' => 'Slow DB', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'db_failed', 'label' => 'Failed DB', 'format' => fn($v): string => admin_gallery_report_n($v)],
        ]) . '<div class="split"><div><h3>Top galleries</h3>' . admin_gallery_report_table($telemetry['top_galleries'] ?? [], [
            ['key' => 'id', 'label' => 'ID'],
            ['key' => 'title', 'label' => 'Title'],
            ['key' => 'page_views', 'label' => 'Page views', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'photo_views', 'label' => 'Photo views', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'photo_seconds', 'label' => 'Photo time', 'format' => fn($v): string => admin_gallery_report_duration((int) $v)],
            ['key' => 'media_bytes', 'label' => 'Media bytes', 'format' => fn($v): string => admin_gallery_report_bytes($v)],
        ]) . '</div><div><h3>Top routes</h3>' . admin_gallery_report_table($telemetry['top_routes'] ?? [], [
            ['key' => 'route_name', 'label' => 'Route'],
            ['key' => 'page_views', 'label' => 'Page views', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'photo_views', 'label' => 'Photo views', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'client_errors', 'label' => 'Errors', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'media_bytes', 'label' => 'Media bytes', 'format' => fn($v): string => admin_gallery_report_bytes($v)],
        ]) . '</div></div><div class="split"><div><h3>Browsers</h3>' . admin_gallery_report_bar_table($telemetry['browsers'] ?? [], 'sessions') . '</div><div><h3>Devices</h3>' . admin_gallery_report_bar_table($telemetry['devices'] ?? [], 'sessions') . '</div></div><h3>Performance</h3>' . admin_gallery_report_table($telemetry['performance'] ?? [], [
            ['key' => 'metric_name', 'label' => 'Metric'],
            ['key' => 'samples', 'label' => 'Samples', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'avg_value', 'label' => 'Average', 'format' => fn($v): string => admin_gallery_report_n($v, 2)],
            ['key' => 'min_value', 'label' => 'Min', 'format' => fn($v): string => admin_gallery_report_n($v, 2)],
            ['key' => 'max_value', 'label' => 'Max', 'format' => fn($v): string => admin_gallery_report_n($v, 2)],
        ]) . '<h3>Client errors</h3>' . admin_gallery_report_table($telemetry['client_errors'] ?? [], [
            ['key' => 'error_kind', 'label' => 'Error kind'],
            ['key' => 'route_name', 'label' => 'Route'],
            ['key' => 'events', 'label' => 'Events', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'last_seen', 'label' => 'Last seen'],
        ]) . '<h3>Database telemetry hot spots</h3>' . admin_gallery_report_table($telemetry['database_summary'] ?? [], [
            ['key' => 'route_name', 'label' => 'Route'],
            ['key' => 'operation', 'label' => 'Operation'],
            ['key' => 'table_name', 'label' => 'Table'],
            ['key' => 'query_count', 'label' => 'Queries', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'failed_count', 'label' => 'Failed', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'slow_count', 'label' => 'Slow', 'format' => fn($v): string => admin_gallery_report_n($v)],
            ['key' => 'latency_ms_sum', 'label' => 'Latency sum', 'format' => fn($v): string => admin_gallery_report_n($v) . ' ms'],
            ['key' => 'latency_ms_max', 'label' => 'Max latency', 'format' => fn($v): string => admin_gallery_report_n($v) . ' ms'],
        ]) . '<h3>Recent telemetry events</h3>' . admin_gallery_report_table($telemetry['recent_events'] ?? [], [
...
function admin_gallery_report_render_logs_section(array $logs): string
{
    if (empty($logs['available'])) {
        return '<section class="panel"><h2>Operational logs</h2><p class="muted">Admin log table is not available.</p></section>';
    }
    return '<section class="panel"><h2>Operational logs</h2><div class="grid">'
        . admin_gallery_report_metric_card('Log records', admin_gallery_report_n($logs['total'] ?? 0), 'all time')
        . admin_gallery_report_metric_card('Last 7 days', admin_gallery_report_n($logs['last_7_days'] ?? 0), 'recent operational events')
        . '</div><div class="split"><div><h3>Levels</h3>' . admin_gallery_report_bar_table($logs['level_rows'] ?? [], 'count') . '</div><div><h3>Categories</h3>' . admin_gallery_report_bar_table($logs['category_rows'] ?? [], 'count') . '</div></div><h3>Top events</h3>' . admin_gallery_report_bar_table($logs['top_events'] ?? [], 'count') . '<h3>Recent errors and critical events</h3>' . admin_gallery_report_table($logs['recent_errors'] ?? [], [
