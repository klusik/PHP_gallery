 ...
 *
 * File: public/assets/gallery-modules/admin-experimental-thumbnail-rebuild.js
 * Module Type: Browser Module
 *
 * Purpose:
 *   Coordinates the experimental browser-assisted thumbnail rebuild workflow.
 *
 * Responsibilities:
...
const workerScriptUrl = new URL('./experimental-upload-worker.js?v=20260610-thumbnail-serial-v5', import.meta.url);
...
export async function runExperimentalThumbnailRebuild(form, progress, options = {}) {
    ...
    const capability = experimentalThumbnailRebuildCapability(config);
    if (!config.enabled || !capability.ok) {
        throw new Error(capability.reason || i18n('admin.experimental_thumbnail_rebuild.unavailable', 'Experimental thumbnail rebuild is not available in this browser.'));
    }

    const sessionId = experimentalThumbnailRebuildSessionId();
    ...
    const preparingLabel = requestedScope === 'missing'
        ? i18n('admin.experimental_thumbnail_rebuild.preparing_missing', 'Preparing browser rebuild for missing thumbnails...')
        : i18n('admin.experimental_thumbnail_rebuild.preparing', 'Preparing browser thumbnail rebuild...');
    updateRebuildProgress(progress, 0, 0, aggregate, preparingLabel);

    const initialPass = await runExperimentalThumbnailRebuildScope(form, progress, config, sessionId, aggregate, requestedScope, batchIndex, 0);
    ...
    for (let repairPass = 1; repairPass <= 3; repairPass += 1) {
        ...
        const repairResult = await runExperimentalThumbnailRebuildScope(form, progress, config, sessionId, aggregate, 'missing', batchIndex, repairPass);
    ...
    const completeLabel = requestedScope === 'missing'
        ? i18n('admin.experimental_thumbnail_rebuild.complete_missing', 'Browser-assisted missing-thumbnail rebuild complete.')
        : i18n('admin.experimental_thumbnail_rebuild.complete', 'Browser-assisted thumbnail rebuild complete.');
...
async function runExperimentalThumbnailRebuildScope(form, progress, config, sessionId, aggregate, scope, startBatchIndex, passIndex) {
    ...
    while (true) {
        ...
        updateRebuildProgress(progress, aggregate.processed, total, aggregate, i18n('admin.experimental_thumbnail_rebuild.downloaded_chunk', 'Downloaded source chunk {current}.', {current: chunkIndex + 1}));
        ...
        if (items.length > 0) {
            ...
            await processSourceItemsWithWorkerPool(sourceItems, config, async (item, completed, count) => {
                ...
                updateRebuildProgress(
                    ...
                    total,
                    aggregate,
                    i18n('admin.experimental_thumbnail_rebuild.processing_chunk', 'Creating thumbnails in the browser for source chunk {current}: {done}/{total}.', {current: chunkIndex + 1, done: completed, total: count})
                );
            });
            ...
            for (const batch of batches) {
                updateRebuildProgress(
                    ...
                    total,
                    aggregate,
                    i18n('admin.experimental_thumbnail_rebuild.uploading_batch', 'Uploading prepared thumbnail batch {current} of {total} for source chunk {chunk}.', {current: batch.localIndex + 1, total: batches.length, chunk: chunkIndex + 1})
                );
        ...
        updateRebuildProgress(progress, aggregate.processed, total, aggregate, i18n('admin.experimental_thumbnail_rebuild.chunk_complete', 'Browser thumbnail source chunk complete.'));
...
function experimentalThumbnailRebuildConfig(form) {
    ...
    if (toggle instanceof HTMLInputElement) {
        try {
            parsed = JSON.parse(toggle.dataset.experimentalThumbnailRebuildConfig || '{}');
        } catch (error) {
            parsed = {};
        }
    }
...
function sourceItemsFromManifest(items, entries, config) {
    ...
    if (missing.length > 0) {
        throw new Error(i18n('admin.experimental_thumbnail_rebuild.source_entry_missing', 'The source ZIP chunk was incomplete. Missing source entries: {items}.', {items: missing.slice(0, 8).join(', ')}));
    }
    return sourceItems;
}
...
function assertPreparedRebuildItemComplete(item, config) {
    ...
    if (variants.length < expected) {
        throw new Error(i18n('admin.experimental_thumbnail_rebuild.incomplete_worker_item', 'Browser worker returned an incomplete thumbnail set for image {image}.', {image: String(item.imageId || 'unknown')}));
    }
}
...
async function downloadSourceChunk(form, config, sessionId, offset, scope = 'all') {
    ...
    if (!response.ok || contentType.includes('application/json') || contentType.includes('text/html') || contentType.includes('text/plain')) {
        const result = await readJsonOrTextResponse(response, i18n('admin.experimental_thumbnail_rebuild.source_failed', 'Source chunk download failed.'));
        throw new Error(result.error || i18n('admin.experimental_thumbnail_rebuild.source_failed', 'Source chunk download failed.'));
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        const assign = (worker) => {
            ...
            activeTimers.set(worker, window.setTimeout(() => {
                rejectOnce(new Error(i18n('admin.experimental_thumbnail_rebuild.worker_timeout', 'Browser thumbnail rebuild worker timed out while processing image {image}.', {image: String(item.imageId || 'unknown')})));
            }, taskTimeoutMs));
            worker.postMessage({
        ...
        for (let workerIndex = 0; workerIndex < workerCount; workerIndex += 1) {
            ...
            worker.addEventListener('message', (event) => {
                ...
                clearWorkerTimer(worker);
                if (!data.ok) {
                    rejectOnce(new Error(data.error || i18n('admin.experimental_thumbnail_rebuild.worker_failed', 'Browser thumbnail rebuild worker failed.')));
                    return;
                }
            ...
            worker.addEventListener('error', (event) => {
                clearWorkerTimer(worker);
                rejectOnce(new Error(event.message || i18n('admin.experimental_thumbnail_rebuild.worker_failed', 'Browser thumbnail rebuild worker failed.')));
            });
            assign(worker);
        }
    });
...
function createThumbnailRebuildBatcher(config, sessionId, chunkIndex, startBatchIndex) {
    ...
    const finalizeCurrent = async () => {
        ...
        if (zipBlob.size > targetBytes) {
            throw new Error(i18n('admin.experimental_thumbnail_rebuild.batch_too_large', 'A prepared thumbnail ZIP batch is larger than the configured upload limit.'));
        }
    ...
    return {
        addItem: async (item) => {
            ...
            if (itemBytes > targetBytes) {
                throw new Error(i18n('admin.experimental_thumbnail_rebuild.item_too_large', 'One prepared thumbnail package is larger than the configured upload limit.'));
            }
...
async function uploadPreparedThumbnailBatchWithRetry(form, config, sessionId, batch, batchIndex, totalBatches) {
    ...
    throw lastError || new Error(i18n('admin.experimental_thumbnail_rebuild.batch_failed', 'Prepared thumbnail batch upload failed.'));
}
...
async function uploadPreparedThumbnailBatch(form, config, sessionId, batch, batchIndex, totalBatches) {
    ...
    body.append('zip_batch', batch.blob, `experimental-thumbnail-rebuild-${sessionId}-${batchIndex}.zip`);
    ...
    const result = await readJsonOrTextResponse(response, i18n('admin.experimental_thumbnail_rebuild.batch_failed', 'Prepared thumbnail batch upload failed.'));
    if (!response.ok || result.ok === false) {
        throw new Error(result.error || i18n('admin.experimental_thumbnail_rebuild.batch_failed', 'Prepared thumbnail batch upload failed.'));
    }
    return result;
}
...
function createStoreOnlyZipInWorker(entries) {
    return workerRoundTrip({action: 'zip', entries}, i18n('admin.experimental_thumbnail_rebuild.zip_failed', 'Prepared thumbnail ZIP packaging failed.')).then((data) => {
        if (!(data.zipBlob instanceof Blob)) {
            throw new Error(i18n('admin.experimental_thumbnail_rebuild.zip_failed', 'Prepared thumbnail ZIP packaging failed.'));
        }
        return data.zipBlob;
    });
}
...
async function readJsonOrTextResponse(response, fallbackMessage) {
    ...
    try {
        ...
        if (snippet.startsWith('<')) {
            ...
            return {ok: false, error: `${fallbackMessage} ${i18n('admin.experimental_thumbnail_rebuild.html_response', 'The server returned HTML instead of JSON. Check the admin logs or PHP error log.')} ${statusText}: ${snippet.slice(0, 120)}`};
        }
