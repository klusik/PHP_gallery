...
$shareModel = (string) file_get_contents($root . '/app/models/viewer_collection_shares.php');
$collectionModel = (string) file_get_contents($root . '/app/models/viewer_collections.php');
$identityAdapter = (string) file_get_contents($root . '/app/bootstrap/viewer_identity_context.php');
$controller = (string) file_get_contents($root . '/' . $controllerPath);
$shareView = (string) file_get_contents($root . '/app/views/viewer_collection_shares.php');
...
viewer_phase30_assert($service !== '' && $controller !== '' && $shareView !== '', 'Dedicated Phase 3.0 service, controller, and view must exist.');
...
$disabledRouteDecision = viewer_phase30_function_source($featureFlags, 'feature_flag_disabled_route_decision');
$disabledRouteTransport = viewer_phase30_function_source($dispatch, 'cms_apply_feature_disabled_route_response');
viewer_phase30_assert(str_contains($disabledRouteDecision, 'if (!$isAdmin)') && str_contains($disabledRouteDecision, "'status' => 404"), 'Anonymous Viewer Accounts routes must remain generic not-found while the master feature is OFF.');
viewer_phase30_assert(str_contains($disabledRouteTransport, 'http_response_code('), 'Disabled feature status emission must remain at the HTTP dispatch boundary.');
...
$ownerSectionView = viewer_phase30_function_source($shareView, 'view_render_viewer_collection_share_owner_section');
viewer_phase30_assert(str_contains($ownerSection, 'viewer_csrf_token()'), 'Owner share controller must issue Viewer CSRF for the view model.');
viewer_phase30_assert(str_contains($ownerSection, 'viewer_collection_share_secret_flash_key($collectionId)') && str_contains($ownerSection, "'new_secret_url' => " . '$newSecretUrl'), 'New share secret must remain collection-scoped show-once controller state.');
viewer_phase30_assert(str_contains($ownerSectionView, 'readonly') && str_contains($ownerSectionView, "'new_secret_url'"), 'Owner share view must render the one-time secret as readonly output.');
viewer_phase30_assert(str_contains($ownerSectionView, 'Replace this share link?') && str_contains($ownerSectionView, 'Revoke this share link?'), 'Replace/revoke owner actions must use simple confirmations in the presentation layer.');
...
viewer_phase30_assert(str_contains($replace, 'viewer_collection_model_transaction(') && str_contains($collectionModel, 'beginTransaction()') && str_contains($collectionModel, 'commit()') && str_contains($collectionModel, 'rollBack()'), 'Create/replace must delegate to the collection model transaction boundary.');
viewer_phase30_assert(strpos($replace, 'viewer_collection_lock_mutation_account') < strpos($replace, 'viewer_collection_lock_owned') && strpos($replace, 'viewer_collection_lock_owned') < strpos($replace, 'viewer_collection_share_model_active_ids_lock'), 'Create/replace lock order must be account -> collection -> share rows.');
viewer_phase30_assert(str_contains($replace, 'viewer_collection_share_model_active_ids_lock(') && str_contains($shareModel, 'WHERE viewer_collection_id = ? AND revoked_at IS NULL ORDER BY id ASC FOR UPDATE'), 'Create/replace must lock all unrevoked share rows in the model.');
viewer_phase30_assert(strpos($replace, 'viewer_collection_share_model_revoke_active(') < strpos($replace, 'viewer_collection_share_model_insert(') && str_contains($shareModel, 'UPDATE viewer_collection_share_tokens SET revoked_at = ?') && str_contains($shareModel, 'INSERT INTO viewer_collection_share_tokens'), 'Replacement must revoke previous unrevoked rows before creating new authority through the model.');
...
viewer_phase30_assert(str_contains($shareModel, '(viewer_collection_id, created_by_viewer_account_id, token_hash, created_at, expires_at)'), 'Only token_hash, not plaintext token, may be persisted.');
viewer_phase30_assert(!preg_match('/INSERT INTO viewer_collection_share_tokens[^;]*(?:\btoken\b)(?!_hash)/is', $shareModel), 'Share INSERT must never persist a plaintext token column.');
...
viewer_phase30_assert(str_contains($accountLock, 'viewer_collection_model_account_lock(') && str_contains($accountLock, 'viewer_account_can_mutate_content($account)') && str_contains($accountLock, 'security_version\'] ?? 0) !== $expectedSecurityVersion') && str_contains($collectionModel, 'must_change_password'), 'Transaction account lock must revalidate mutable-account policy and security_version against the locked model row.');
...
viewer_phase30_assert(str_contains($revoke, 'viewer_collection_model_transaction(') && str_contains($revoke, 'viewer_collection_lock_mutation_account') && str_contains($revoke, 'viewer_collection_lock_owned') && str_contains($revoke, 'viewer_collection_share_model_active_ids_lock('), 'Revocation must revalidate owner/account/share state transactionally.');
viewer_phase30_assert(str_contains($revoke, 'viewer_collection_share_model_revoke_active(') && str_contains($shareModel, 'UPDATE viewer_collection_share_tokens SET revoked_at = ?'), 'Revocation must mark share authority revoked through the model.');
viewer_phase30_assert(!preg_match('/DELETE\s+FROM\s+(viewer_collections|viewer_collection_items|viewer_favourites|images|galleries)/i', $shareModel), 'Revocation persistence must not delete collection state, favourites, or source media.');
...
viewer_phase30_assert(strpos($exchange, 'viewer_collection_share_token_syntax_valid($token)') < strpos($exchange, 'viewer_collection_shares_storage_available()') && strpos($exchange, 'viewer_collection_shares_storage_available()') < strpos($exchange, 'security_authority_token_hash($token)') && strpos($exchange, 'security_authority_token_hash($token)') < strpos($exchange, 'viewer_collection_share_model_candidate('), 'Malformed tokens must be rejected before model lookup.');
viewer_phase30_assert(str_contains($exchange, 'viewer_collection_share_model_lock(') && str_contains($shareModel, 'token_hash = ?') && str_contains($shareModel, 'revoked_at') && str_contains($exchange, 'viewer_collection_share_expiry_active'), 'Exchange must revalidate the exact durable share and its revoke/expiry state through the model.');
viewer_phase30_assert(str_contains($exchange, 'viewer_collection_model_account_lock(') && str_contains($exchange, 'viewer_account_can_authenticate($account)') && str_contains($collectionModel, 'FROM viewer_accounts WHERE id = ? LIMIT 1 FOR UPDATE'), 'Exchange must lock and reject missing/suspended/disabled owners.');
viewer_phase30_assert(str_contains($exchange, 'viewer_collection_model_owned_lock(') && str_contains($collectionModel, 'FROM viewer_collections WHERE id = ? AND viewer_account_id = ? LIMIT 1 FOR UPDATE'), 'Exchange must verify collection still exists under the share owner.');
viewer_phase30_assert(str_contains($exchange, 'viewer_identity_session_regenerate()') && str_contains($identityAdapter, 'session_regenerate_id(true)'), 'Bearer capability exchange must rotate the PHP session id through the Core adapter.');
...
viewer_phase30_assert(str_contains($exchange, 'viewer_collection_share_model_touch_last_used(') && str_contains($exchange, 'operational telemetry') && str_contains($shareModel, 'last_used_at'), 'last_used_at must remain best-effort operational metadata persisted by the model.');
...
viewer_phase30_assert(!str_contains($exchange, 'viewer_collection_share_model_revoke_active('), 'Scanner/human GET exchange must not consume or revoke the reusable link.');
...
$sharedView = viewer_phase30_function_source($shareView, 'view_render_viewer_collection_shared');
viewer_phase30_assert(str_contains($sharedController, 'viewer_collection_share_public_headers()') && str_contains($sharedController, 'view_public_noindex_meta()') && str_contains($httpView, '<meta name="robots" content="noindex,nofollow">'), 'Clean shared page must remain no-store/noindex defense in depth through the shared HTTP view.');
...
viewer_phase30_assert(str_contains($authorize, 'viewer_collection_share_model_authorization_row(') && str_contains($shareModel, 'WHERE vcs.id = ? AND vcs.viewer_collection_id = ?') && str_contains($shareModel, 'vcs.revoked_at') && str_contains($authorize, 'viewer_collection_share_expiry_active') && str_contains($authorize, 'viewer_account_can_authenticate($row)'), 'Every clean shared view must revalidate durable share, expiry, collection owner, and active account state.');
...
viewer_phase30_assert(strpos($sharedRead, 'viewer_collection_share_session_authorize($collectionId)') < strpos($sharedRead, 'viewer_collection_share_model_collection($collectionId)'), 'Collection metadata must not load before matching session-grant revalidation.');
viewer_phase30_assert(str_contains($sharedRead, 'viewer_collection_share_model_references(') && str_contains($shareModel, 'SELECT image_id, position, created_at FROM viewer_collection_items') && !str_contains($shareModel, 'filename') && !str_contains($shareModel, 'relative_path'), 'Shared read model must return references only, not source metadata or paths.');
...
viewer_phase30_assert(str_contains($sourceResolver, 'image_model_public_rows_by_ids(') && str_contains($imagesModel, 'AND visibility = ?') && str_contains($imagesModel, "['public']"), 'Shared rendering must require current public image visibility before source authorization.');
...
viewer_phase30_assert(str_contains($sharedView, 'e((string) ($viewModel[\'title\'] ?? \'\'))'), 'Malicious collection titles must be HTML-escaped by the recipient view.');
viewer_phase30_assert(!str_contains($sharedController, 'viewer email') && !str_contains($sharedController, "['email']") && !str_contains($sharedController, 'token_hash') && !str_contains($sharedController, 'relative_path') && !str_contains($sharedController, 'filename'), 'Recipient controller must not expose owner identity, token hash, or hidden path/filename metadata.');
viewer_phase30_assert(str_contains($sharedController, '$hiddenCount') && str_contains($sharedView, 'some_unavailable'), 'Denied items may only produce generic unavailability feedback prepared by the controller and rendered by the view.');
viewer_phase30_assert(!str_contains($sharedView, '<form') && !str_contains($sharedView, 'viewer_favourite') && !str_contains($sharedView, 'render_viewer_collection_add_control_html'), 'Shared collection view must remain read-only and must not expose owner/favourite mutation controls.');
...
viewer_phase30_assert(str_contains($transition, 'viewer_account_model_revoke_transition_authority(') && str_contains($accountModel, 'UPDATE viewer_collection_share_tokens SET revoked_at = ?'), 'Suspension/disable transition must revoke collection-share capabilities.');
viewer_phase30_assert(!preg_match('/UPDATE viewer_collection_share_tokens SET revoked_at\s*=\s*NULL/i', $transition . "\n" . $accountModel), 'Restoration must never clear share revoked_at.');
...
viewer_phase30_assert(str_contains($accountDelete, 'viewer_lifecycle_model_revoke_created_shares(') && str_contains($accountDelete, 'viewer_lifecycle_model_account_delete(') && str_contains($lifecycleModel, 'UPDATE viewer_collection_share_tokens SET revoked_at = ?') && str_contains($lifecycleModel, 'DELETE FROM viewer_accounts'), 'Viewer account deletion must invalidate shares then use existing account/FK cleanup through the lifecycle model.');
$adminDelete = viewer_phase30_function_source($adminAccountsService, 'viewer_admin_account_delete');
viewer_phase30_assert(str_contains($adminDelete, 'viewer_account_model_admin_delete(') && str_contains($accountModel, 'UPDATE viewer_collection_share_tokens SET revoked_at = ?') && str_contains($accountModel, 'DELETE FROM viewer_accounts'), 'Admin viewer deletion must invalidate shares then use existing account/FK cleanup.');
