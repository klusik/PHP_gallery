...
12. Enter fullscreen or mobile fullscreen and trigger a full-quality load (passive 100% promotion or deliberate zoom) on a
   photograph large enough to transfer for a visible duration, throttling the network if needed. Confirm a progress bar
   with a percentage and a dynamically unit-scaled byte readout (for example `8.4 MB / 22.7 MB`) appears at the bottom of
   the stage, fills smoothly toward 100%, and disappears once the sharper image is installed. Confirm normal (non-fullscreen)
   lightbox mode keeps only the existing compact loading pill/ring and never shows the byte-metrics bar. With Network tools
