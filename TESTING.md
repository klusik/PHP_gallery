...
`tests/maintenance_center_test.php` is the focused no-database contract for the central Maintenance Center. The central PHP regression suite executes it automatically. It covers registry keys/dependencies, deterministic pipeline order, state-machine transitions, optional deep-media selection, weighted monotonic progress, persistence/lock schema, read-only Analyze boundaries, stale-plan revision inputs, one-table-per-step ANALYZE/OPTIMIZE wiring, current eligibility rechecks, cooperative cancellation evidence, automatic site-maintenance lock precedence, independent download/cache operation order and failure isolation, bounded exception classification and raw-message redaction, POST/CSRF/Admin mutation-envelope contracts, route wiring, browser continuation without reload, Dashboard/navigation integration, and Maintenance Center translation-key parity across English, Czech, German, and Swedish.
