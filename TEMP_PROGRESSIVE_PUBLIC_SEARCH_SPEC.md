...
Status: implementation complete; optional future acceleration stages deferred  
...
The staged implementation is now present in the source tree through the Stage 7 MVC/UX-stability checkpoint:

- Stage 1 search-domain contracts and centralized relevance scores are implemented in `app/services/public_search_progressive.php`.
- Stage 2 fast `primary` gallery/title/tag search is implemented with bounded candidate discovery and post-limit hydration. The primary candidate query does not join `images`, image tags, descriptions, or AI metadata.
- Stage 3 browser orchestration uses stable result keys, deterministic relevance merging, query-generation tracking, independent abort/timer ownership, stale-response suppression, and a compact 14-result visible budget with category caps.
- Stage 4 lightweight `media` search discovers direct photo matches from filename/title and image tags without scanning descriptions, AI metadata, gallery tags, or parent-gallery text. Parent gallery data is only authorization/display context.
- Stage 5 `descriptive` and `deep` phases are implemented. Gallery descriptions, gallery-tag descriptions, and translated gallery titles are deferred to `descriptive`; translated gallery descriptions, image descriptions, image-tag descriptions, translated image title/description, and optional AI `searchable_text` are isolated in later `deep` candidate queries.
- Each Stage 5 source uses a separate bounded candidate query. Description, translation, tag-description, and AI sources are not joined into one broad intermediate row set before ranking.
- Normal browser live search now schedules `primary` immediately after input debounce, `media` after 100 ms, `descriptive` after 300 ms, and `deep` after 550 ms. Query changes clear the deferred timers before they dispatch where possible and still discard stale responses by generation if a request was already in flight.
- The browser no longer schedules the monolithic no-phase compatibility request during normal live search. The no-phase endpoint remains available for backward compatibility while progressive phases become the primary browser path.
- Late descriptive/deep failures settle independently. Successful earlier results remain visible and a deep failure does not replace them with a global error state.
- The compatibility image query remains narrowed so parent gallery title/description/tags and translated gallery metadata do not manufacture photo matches. The old no-phase implementation remains available to external/legacy callers but is no longer on the live-search critical path.
- Stage 6 introduces an explicit MVC boundary for progressive search. `app/models/public_search_progressive.php` and its `deferred.php` part now own progressive SQL/data access; services own relevance/orchestration/localization/result shaping; `app/controllers/public_search.php` owns the JSON endpoint; and `app/views/public_search.php` owns search-bar HTML.
- Stage 6 splits primary gallery title and gallery-tag discovery into separate bounded model queries instead of one LEFT JOIN aggregate. Media image-tag matching is reduced to one score per image before image/gallery authorization joins. Gallery-tag-description, image-tag-description, and AI matches similarly reduce relation rows before joining entity rows.
- Primary/media/deferred hydration aggregation remains post-candidate and bounded. Progressive service modules no longer call PDO or construct SQL directly.
- The historical no-phase compatibility search now follows the same MVC ownership rule: `app/models/public_search.php` owns its SQL/PDO work while `app/services/public_search.php` retains feature/schema policy, localization, and result shaping. The normal browser flow still does not schedule this compatibility path.
- Stage 7 keeps globally relevance-ranked presentation because the score bands already encode the intended gallery/title/tag-first behavior, but avoids speculative weight changes without production query evidence. Stable keyed result DOM nodes are reconciled rather than recreating the whole list after every phase, preserving focus and reducing visual flicker.
- Stage 7 adds subtle insertion feedback for newly arriving results, a `prefers-reduced-motion` override, stable `aria-busy` state, and polite status semantics. The obsolete browser helper for the monolithic compatibility request has been removed.
- Stage 0A production measurement tooling is implemented in Admin as Search diagnostics. It repeats the four progressive phases, captures model-query timings and row counts, inventories relevant table/index metadata, runs `EXPLAIN FORMAT=JSON` with a classic `EXPLAIN` fallback, and exports one portable JSON report.
- Production validation was completed on 2026-09-13 against MariaDB 10.11.18 with approximately 497 galleries, 30,404 images, 296 tags, 1,182 gallery-tag relations, 94 image-tag relations, and 66 AI metadata rows.
- Representative production queries `320`, `letadla`, `OFP`, and a negative `Boeing` query confirmed the intended relevance behavior and progressive phase separation. Warm phase latency is generally in the low single-digit to low tens of milliseconds. The largest measured cold-cache phase was `media` for `letadla` at approximately 163 ms, which is acceptable for the deferred progressive path.
- Production `EXPLAIN` evidence confirms that the remaining `%query%` substring scans can require filesort/table work, especially for image filename/title and description matching, but the measured cost does not justify introducing a denormalized search index, FULLTEXT migration, or n-gram subsystem at the current dataset size.
- Stage 8, Stage 9, and Stage 10 therefore remain optional future scalability paths, not unfinished work. They should only be reconsidered after a future Search diagnostics run demonstrates material regression as the dataset or AI metadata volume grows.

The core progressive-search feature is considered complete as of 2026-09-13. The no-phase endpoint remains as a compatibility contract, normal browser search uses the progressive path, MVC ownership is enforced across public search and diagnostics, and production measurements support stopping further optimization because expected gains are now subject to diminishing returns.
...
Implementation status: **complete. Admin diagnostic infrastructure and target-host production measurements completed 2026-09-13.** The current tool intentionally uses ordinary `EXPLAIN`, not `EXPLAIN ANALYZE`, so optimizer inspection does not execute every captured statement a second time on shared hosting.
...
- collect `EXPLAIN FORMAT=JSON` with classic `EXPLAIN` fallback for representative queries; evaluate `EXPLAIN ANALYZE` separately only if target-host safety and runtime justify executing the statement again,
...
Deliverable: Admin > Maintenance > Search diagnostics provides diagnostic support and portable JSON export. Production reports for `320`, `letadla`, `OFP`, and negative `Boeing` were reviewed on the target database; no search-index or FULLTEXT migration is justified at the current scale.
...
Implementation status: **complete, including production query-plan measurement.**
...
Implementation checkpoint: **complete. Code-side cleanup and production plan/index measurement completed 2026-09-13. No additional index migration is justified by current measurements.** Progressive SQL now resides in the model layer and relation-heavy tag/AI sources are reduced before entity/gallery joins where practical. No speculative index migration has been added without target-host query plans.
...
Implementation checkpoint: **complete for the public-search feature. Code-side UX/MVC work and representative production relevance validation completed 2026-09-13. No further relevance-weight changes are justified by the measured queries.** The browser reconciles stable keyed result nodes instead of rebuilding the complete list, new staged results receive subtle reduced-motion-safe insertion feedback, and busy/status accessibility state is explicit. Global relevance ordering and the 14/8/8 result budgets remain unchanged because the representative production queries validated the intended ordering and no tuning benefit was demonstrated. The no-phase compatibility SQL has also moved into `app/models/public_search.php`, so all public-search service modules are now free of PDO/SQL ownership.
...
### Feature closure decision - 2026-09-13

Production diagnostics show that the progressive design has removed the original need for further immediate search-engine work. The current implementation is accepted as complete.

Closure evidence:

- `320` returns the intended `A320` gallery through the high-relevance primary path before deferred image/deep work.
- `letadla` validates gallery-tag and image-tag relevance ordering.
- `OFP` validates that description-only content is still found in the deferred descriptive phase.
- negative `Boeing` validates that the progressive split does not manufacture false matches.
- cold-cache cost is bounded and acceptable for deferred phases; warm execution is substantially faster.
- no production evidence currently supports the operational complexity of a derived search-document table, FULLTEXT migration, or n-gram index.

The Search diagnostics admin tool remains part of the project and is the trigger for revisiting scalability work later. If future reports show materially higher latency due to image count, tag volume, translations, or widespread AI metadata, resume at Stage 8 rather than redesigning the completed progressive flow.
...
The core redesign, excluding optional future index/FULLTEXT/n-gram stages, is complete when all of the following are true. **This definition of done is considered satisfied as of 2026-09-13 based on source audits and production Search diagnostics; optional Stages 8 to 10 are explicitly deferred.**
...
- representative target-host search latency and optimizer plans are measured after the change; exact pre-instrumentation timings were not available, so the historical baseline is the documented monolithic query shape and observed slow user experience rather than a fabricated numeric value,
