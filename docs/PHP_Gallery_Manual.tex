...
\newcommand{\version}{0.104}
\newcommand{\manualdate}{19 September 2026}
...
As~you type at~least two characters in~the~title field, Admin may~show the~remaining text of~a~matching existing gallery as~a~faint inline suggestion. Titles under the~selected parent are~preferred, with the~newest matching sibling first; a~matching title elsewhere can~be~used when no~sibling matches. Press \codeword{Tab} or~\codeword{Right Arrow}, or~select the~suggested text, to~accept it. Continue typing or~press \codeword{Escape} to~ignore it. The~suggestion changes only the~ordinary title field and~does not~save, rename, or~modify any existing gallery.
