...
\subsection{What is~new in~Version 0.91.1}
\index{Version 0.91.1}

Version~0.91.1 is~a~focused lightbox navigation refinement. The~ordinary left and~right arrow keys continue to~move one photograph at~a~time; holding Shift while pressing either arrow moves ten photographs in~the~same direction. The~shortcut is~announced in~the~translated lightbox help text and~works with~the~existing paginated gallery, Smart Gallery, zoom, fullscreen, map, slideshow, access-control, and~no-JavaScript-compatible viewer behavior.

The~release also corrects backwards ten-photo movement at~the~beginning of~a~gallery by~using stable modular navigation. No~database migration or~filesystem change is~required.
