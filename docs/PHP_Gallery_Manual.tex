...
Version~\version{} adds the~image-move journal, gallery edit revision, and~administrator operation-key ledger through ordinary table/column migrations. Run them from the~Gallery administration page as~usual. They require the~installation's normal schema-change authority, but~do~not~require database triggers, stored routines, \codeword{SUPER}, a~global server-variable change, or~separate database-administrator intervention. If~an~interrupted attempt already added the~revision column without recording the~migration, using the~same Gallery migration action again completes the~ledger safely.
