...
Version~\version{} adds the~\codeword{maintenance\_jobs} table through an~ordinary migration. Run it~from the~Gallery administration page as~usual. It~requires the~installation's normal table-creation authority, but~does~not~require database triggers, stored routines, \codeword{SUPER}, a~global server-variable change, or~separate database-administrator intervention. Until the~migration is~available and~verified, the~Maintenance Center refuses to~start a~job instead of~attempting maintenance without durable checkpoints.
