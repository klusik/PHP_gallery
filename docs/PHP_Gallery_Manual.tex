...
% File: docs/PHP_Gallery_Manual.tex
% Module Type: LaTeX Product Manual
% Purpose: Explain administrator workflows, operation and maintenance of PHP Gallery.
% Responsibilities:
%   - Maintain the usage guide and technical reference while preserving reproducible PDF typography.
...
\makeatletter
% Keep two-digit subsection numbers separated from their table-of-contents titles.
...
\newcommand{\version}{0.108.1}
\newcommand{\manualdate}{25 September 2026}
...
Use the~\ui{+} control on~a~public gallery or~\ui{Create gallery here} in~its editor to~start a~child gallery. The~right-side Admin panel asks only for~the~new title; it~already knows the~parent gallery that supplied the~control. The~new gallery starts \ui{Unpublished}. After creation, the~same panel opens its complete editor for~description, date, language, tags, access, display, media, and~photographs. The~public page does~not~navigate away during this enhanced workflow.

To~start from~the~standalone Admin create page, enter a~title and~optionally provide SimBrief details, description, source language, and~tags before submitting. Its closed \ui{More options} section contains the~folder name, initial visibility, date range, parent, voting, filename display, and~contained-picture badge. A~short summary below it~shows the~current visibility and~parent. The~dedicated create-and-upload form remains available when photographs should be~uploaded during creation. On~a~validation error, submitted non-secret values and~parent context stay available for~correction.

As~you type at~least two characters in~the~title field, Admin may~show the~remaining text of~a~matching existing gallery as~a~faint inline suggestion. Recent titles under the~selected parent are~preferred; matches elsewhere are~considered only after the~bounded sibling search is~exhausted. Older titles can~be~omitted, so~no~suggestion does~not~prove that a~title is~unique. The~form requests at~most eight candidates instead of~loading the~complete title catalog.

With the~caret at~the~end and~no~text selected, press unmodified \codeword{Tab} or~\ui{Right Arrow}, or~select the~suggested text, to~accept it. Modified shortcuts and~input-method composition keep their ordinary behavior. The~first \codeword{Escape} dismisses completion; a~later press can~reach the~surrounding panel. Translated help and~a~polite announcement make suggestions discoverable without changing the~field name.

Unicode matching accepts only safe whole-character boundaries. Some non-ASCII suggestions are~omitted when server extensions or~browser segmentation support are~unavailable. A~slow, failed, or~outdated lookup never prevents ordinary typing. Acceptance changes only the~title field: it~does~not~submit the~form, rename an~existing gallery, or~alter its access rules. The~same behavior applies after a~side-panel form is~replaced. See \filepath{docs/TITLE_COMPLETION.md} for~matching limits and~technical details.

After the~name-only creation, the~gallery is~already a~real collection in~the~gallery tree, even before additional editor fields are~saved. Its physical folder can~receive photographs through the~browser, filesystem transfer, discovery, or~another supported upload workflow. Fill and~save the~editor fields before publishing. The~database stores descriptive and~presentation settings; the~normal access policy determines what visitors can~see.

Each generated create or~classic-upload form also carries a~private operation identity. If~a~connection fails after the~server may~have finished the~work, retry the~same form instead of~opening a~new one: the~same administrator, operation, and~content receive the~original completed result. A~freshly opened form represents a~new intentional operation and~can~still create a~deliberate duplicate. This identity is~not~a~password or~a~replacement for~sign-in and~CSRF protection.

\subsubsection{Creation sequence}

\begin{enumerate}
  \item Open the~intended parent and~choose its \ui{+} control, or~start a~root gallery from~Admin.
  \item Enter a~visitor-friendly title; optionally accept a~title suggestion.
  \item Create the~Unpublished gallery. The~full editor opens in~the~same panel when JavaScript is~available.
  \item Add the~description, date or~date range, source language, tags, and~optional SimBrief data in~\ui{Identity}.
  \item Review \ui{Access}, \ui{Display}, \ui{Media}, and~the~gallery folder/parent values in~Advanced where relevant.
  \item Save the~gallery, add photographs, and~choose a~cover after suitable photographs exist.
  \item Publish when ready, then preview the~result anonymously.
\end{enumerate}

\subsubsection{Remembered defaults and~SimBrief drafts}
\index{gallery creation!remembered defaults}
\index{SimBrief!pre-create draft}

The~editor can~remember the~current SimBrief identifier and~source language for~the~signed-in administrator. Select the~nearby \ui{Remember for~future galleries} choice only for~a~value you~want to~reuse. Later galleries show that value already selected with a~small note saying it~came from~your saved defaults. The~value remains editable; leaving Remember unchecked preserves the~previous saved choice. Defaults belong to~one administrator and~do not~publish credentials or~copy access rules. The~normal migration action must apply \codeword{202609250001\_gallery\_creation\_preferences.php} before Remember can~save a~new default; gallery creation still works without that optional table.

The~visible SimBrief field accepts a~Pilot ID or~pilot name. Digits alone are~treated as~a~Pilot ID, while other text is~treated as~a~name. On~the~direct creation form, \ui{Generate description draft} can~fetch the~latest OFP before a~gallery exists. Review and~edit the~proposed description before saving. The~import keeps a~private reference for~30~minutes, limited to~the~same administrator and~session; changing the~identifier invalidates that reference. The~OFP and~available route coordinates are~attached only after the~gallery has~been created. If~an~optional attachment or~default save fails after creation, the~gallery stays created and~the~panel reports the~specific warning. Re-import when the~draft expires or~the~session changes.

\subsubsection{Working in~the~compact editor}
\index{gallery editor!side panel}

The~ordinary work areas are~\ui{Identity}, \ui{API}, \ui{Access}, \ui{Display}, and~\ui{Media}; image and~specialized tools follow as~available. The~right-side panel keeps \ui{Save gallery} visible at~its bottom while the~settings form scrolls. Changing tabs does~not~save automatically. Use the~button after editing, then reopen the~gallery if~you~want to~verify the~stored values. Longer explanations sit behind keyboard-accessible question-mark controls, and~less frequent choices sit under \ui{Advanced} disclosures. A~direct Admin editor follows the~same fields, while a~browser without JavaScript uses the~normal form submission.

\ui{Identity} keeps title, description, date, source language, SimBrief, and~tags readily available. Its Advanced section includes slug, folder, parent, sort order, and~AI gallery text when enabled. \ui{API} displays a~copyable upload endpoint, the~key label/action, and~a~newly generated key only once; existing keys appear only when there are~any. AI metadata regeneration, migration, and~the~global API manager live under \ui{Advanced API tools}. \ui{Access} groups visibility, password, share-link controls, and~the~NSFW checkbox, with longer policy explanations under question marks. \ui{Display} starts with the~grid and~common voting/filename switches; EXIF/GPS, gallery-card format, count badge, lightbox mode, Picture Game, thumbnail bounds, and~flight-map text are~under \ui{Advanced display settings}.
...
The~complete editor records the~exact gallery revision that supplied its fields. If~another tab, administrator, upload, move, scan, or~maintenance workflow changes the~gallery first, an~older save is~refused instead of~silently overwriting the~newer state. The~conflict view preserves entered non-secret text and~shows a~safe current comparison. Review and~copy the~intended changes into the~latest editor; re-enter passwords and~reselect files because private values are~never copied into conflict details.
...
The~gallery editor is~the~authoritative place for~settings that belong to~one physical gallery. The~ordinary tabs are~Identity, API, Access, Display, and~Media, followed by~Images and~optional Organizer/Renamer tools where available. Advanced disclosures retain infrequent settings without removing them from~the~shared save form. The~following table summarizes the~current gallery-owned controls in~Version~\version{}.
...
Identity & SimBrief identifier and~defaults & A~single visible field accepts numeric Pilot ID or~pilot name. The~administrator may~remember that identifier and~the~source language for~future galleries; pre-filled values are~marked.\\
API & Upload endpoint and~keys & Copy the~gallery-scoped endpoint, generate a~key with a~label, and~copy the~new raw value immediately. Active keys appear only when present; the~raw value is~not~shown again. AI regeneration and~migration are~under Advanced.\\
...
Display & Picture Game & Advanced option for~the~comparison game in~this gallery branch. Enabling the~game also requires image voting to~be~available.\\
...
Display & Custom grid & First visible Display control: optional columns and~rows for~this gallery. Without an~explicit grid, the~nearest parent grid that permits subgallery inheritance applies, then Theme.\\
...
Display & Flight route & Advanced manual route text supports local identifier lookup and~explicit \codeword{NAME@latitude,longitude} points. SimBrief can~save an~OFP, description draft, and~resolved route coordinates.\\
...
Images & Create all thumbnails & Rebuilds thumbnails for~this gallery. Select \ui{Include subgalleries} to~cover its descendants; other gallery branches remain outside the~job. Browser generation is~selected by~default when available, with a~server-generation option.\\
...
Moving photographs between galleries uses a~durable operation journal. The~Organizer physically moves each original and~verifies its recorded identity before changing catalog ownership. If~a~worker or~connection stops partway through, System Health and~Runtime Diagnostics show a~bounded action state without exposing private paths. A~failed move gives a~safe reason on~screen; trusted Admin logs retain more detailed private diagnostics. Stop competing storage work, preserve a~backup, and~follow \filepath{docs/IMAGE_MOVE_RECOVERY.md}; recovery retains changed or~unverifiable files instead of~overwriting them.
...
The~gallery trash bin is~enabled by~default. Deleting a~gallery through a~normal administrator gallery-delete action moves the~selected folder and~its complete descendant tree out~of~the~live gallery root, records the~metadata required for~recovery, and~lists the~entry under \ui{Maintenance > Trash}. Public pages stop discovering that subtree immediately, but~an~administrator can~restore it~to~its~original path while that path remains free. Restore never overwrites a~new live gallery occupying the~same location.

From the~Trash panel, choose \ui{Restore}, \ui{Delete permanently}, or~\ui{Empty Trash}. Permanent deletion and~Empty Trash cannot~be~undone. Empty Trash works in~bounded batches so~large trash collections do~not~require one unbounded request. Individual-photo deletion is~not~part of~this gallery-level recovery feature and~remains permanent. A~backup remains necessary protection against storage failure, administrative permanent purge, or~damage outside PHP Gallery.
...
The~public gallery itself also exposes contextual Admin controls while a~normal administrator session is~active. Gallery and~photo pencil controls open the~same full editors in~the~side panel. Compact gallery removal uses the~recoverable trash workflow when Trash is~enabled; disabling Trash returns future gallery removals to~the~legacy permanent subtree-deletion service. Compact photo removal is~deliberately labeled \ui{Remove photo ... from CMS}: that quick public-page action removes the~catalog image row and~does not~run the~full source-file deletion service. Use the~normal photo/bulk deletion workflow when the~original file and~generated derivatives should also be~deleted. Anonymous-preview mode hides these contextual Admin controls.
...
\subsection{Reliable in-place side-panel changes}
\index{administration!side-panel mutations}
\index{side panel!mutation completion}
...
Only the~newest panel-open request may~replace its contents; closing or~opening another item cancels ownership by~an~older response. Keyboard focus remains inside the~open drawer and~returns to~the~control that opened it. Unsaved ordinary text can~be~held in~bounded browser memory for~the~current page and~restored when the~same form returns. Passwords, tokens, API keys, file inputs, operation identities, and~other private values are~excluded, and~nothing is~persisted in~browser storage.
...
When the~optional \ui{Picture Manager} feature is~enabled, a~normal signed-in administrator can~select photographs and~direct physical subgalleries in~the~public gallery grid. Checkmarks select individual cards; Shift-click selects a~range; Ctrl-click or~Cmd-click toggles a~card; \ui{Select all}, \ui{Clear}, and~Ctrl/Cmd+A support larger visible-page selections. Smart Gallery cards are~not~part of~this file-based selection. Dragging an~unselected photo starts with that photo as~the~selection, while dragging an~existing photo selection moves the~selected photographs together. A~visible child-gallery card becomes a~drop target, and~dropping there performs the~same validated photo move as~the~destination picker. Physical gallery cards themselves are~not~drag sources.

From the~toolbar the~administrator can~delete a~mixed selection or~create a~real physical gallery from it. Creation accepts a~searchable parent-gallery choice, copies selected photographs into the~new root, and~copies selected physical subgallery trees below it while leaving the~sources unchanged. The~server writes current gallery sidecars, rejects recursive destinations and~existing target folders, reserves each target root before copying, rebuilds copied gallery rows from~the~filesystem, and~removes the~new partial result if~the~operation fails. Mixed deletion removes selected photographs through normal image cleanup and~applies the~configured recoverable Trash policy to~selected gallery trees when available.

Move and~copy-to-existing-gallery remain photo-only actions. If~a~physical gallery is~selected, clear it~before using those controls. Photo selections can~also be~shared or~downloaded. \ui{Share} fetches the~authorized browser-displayable image versions as~browser \codeword{File} objects and~opens the~native Web Share sheet when the~browser/device supports multi-file sharing. The~browser chooses the~available receiving apps, so~PHP Gallery cannot force Instagram Story, Reel, or~another specific target. If~native sharing is~unavailable, cannot accept the~files, or~fails after preparation, the~manager starts the~authenticated selected-photo ZIP download as~a~fallback. The~same ZIP endpoint can~therefore serve both an~explicit selected-photo download and~share fallback. Ordinary visitors never receive these controls, and~the~server revalidates the~logged-in account, CSRF token, source ownership, direct-child gallery ownership, and~destination gallery for~every mutation or~ZIP request.
...
Compressed file size alone does~not~prove that an~image is~safe to~decode. Before GD or~optional Imagick allocates a~full raster, PHP Gallery checks the~format, dimensions, pixel arithmetic, configured/runtime memory budget, and~pipeline limits. A~file that cannot~be~admitted safely is~refused before its original or~generated derivatives are~committed to~the~target gallery. This policy applies to~classic and~browser-prepared ingestion; it~does not~claim that every external RAW decoder has~the~same in-process memory model.
...
\ui{Browser-side thumbnail rebuild} is~selected by~default for~these regeneration actions when browser processing is~available. Clear its checkbox to~use server generation. The~server sends original files to~the~authenticated browser in~bounded source ZIP chunks, the~browser generates thumbnail variants, and~prepared thumbnail ZIP batches are~uploaded back for~server validation and~installation. The~source-chunk and~batch limits are~configured with the~browser-upload settings so~the~workflow does~not~depend on~one oversized PHP request.
...
While a~new photograph is~being prepared, the~viewer uses the~same progress area that reports full-quality loading. Rapid forward or~backward movement always gives the~newest requested photograph ownership of~that indicator, so~an~older preview or~metadata request cannot leave the~viewer permanently busy or~replace the~current selection. If~a~photograph cannot be~presented, the~progress area shows a~recoverable failure message; the~visitor can~continue with the~forward, backward, close, or~reopen controls. These loading states do~not~change gallery access, protected-media, Smart Gallery, map, slideshow, or~no-JavaScript rules.
...
Zooming above 100\% immediately requests the~protected full-quality source for~the~active photograph. The~existing preview remains visible until the~larger image is~ready, and~the~current zoom and~pan position are~preserved. In~normal, full-screen, and~mobile full-screen modes, this transfer shows the~same progress bar with a~percentage and~a~byte count. Changing photographs, closing the~viewer, or~starting a~newer full-quality request cancels the~previous transfer. A~failed or~cancelled full-quality request leaves the~protected preview in~place.
...
Flight-oriented collections can~use the~optional SimBrief integration to~retrieve the~latest operational flight plan for~a~Pilot ID or~pilot name. The~compact editor accepts one visible identifier: digits alone select the~Pilot ID path; other text selects the~pilot-name path. Existing integrations that submit separate fields remain compatible, with Pilot ID preferred when both are~provided. The~editor creates an~editable Markdown description draft, saves the~OFP with the~gallery, and~writes available route coordinates into the~gallery flight-map record. A~failed SimBrief request leaves the~ordinary description field and~manual route editor available. The~pre-create draft is~private and~temporary as~described in~the~gallery creation section.
...
\subsection{Viewer accounts and~invite-only registration}
\index{viewer accounts}
\index{viewer invitations}
\index{viewer account management}
\index{viewer login}
...
The~complete viewer-account subsystem is~wrapped by~a~master switch in~\ui{Admin > Features}. This~master feature is~disabled by~default. While it~is~off, the~public gallery shows no~viewer Login/Account controls, viewer routes are~unavailable, and~the~\ui{Viewer accounts} entry is~hidden from~the~Admin Account menu. Existing viewer identities, favourites, and~private collections remain stored; the~switch controls reachability rather than~deleting data.

After enabling the~master feature, the~administrator can~open \ui{Account > Viewer accounts}. That~page retains the~separate \ui{Viewer accounts (invite-only)} mode switch. The~mode switch controls viewer login/invitations inside the~enabled subsystem, while the~master Features switch controls whether the~subsystem is~exposed at~all.

An~administrator manages viewer identities from \ui{Account > Viewer accounts}. The~existing invite-only workflow remains available: the~administrator may optionally bind an~invitation to~an~intended email address and~copy its~show-once secret link. Opening the~link does~not~create an~account. The~recipient submits an~email address, receives a~verification message through the~configured mail transport, confirms the~verification in~the~browser, and~then chooses a~password of~at~least 15~characters. There is~no~general \ui{Register} or~\ui{Sign up} page.

The~same Admin page can~also create a~viewer account directly. Enter the~viewer email and~either provide a~temporary password that meets the~viewer password policy or~leave the~password blank so~the~gallery generates a~strong one. The~temporary password is~shown once after creation. If~the~optional account-created notification is~sent, the~message contains the~trusted login address and~instructions but~never the~temporary password; provide that password separately through a~trusted channel. With the~master feature enabled, direct account creation remains available while the~subordinate viewer frontend mode is~disabled so~accounts can~be~prepared in~advance, but those viewers cannot sign in~until that~viewer mode is~enabled.

A~directly created viewer cannot use the~temporary password as~a~durable login. After the~first successful credential check, the~browser is~sent to~the~private first-login page and~must choose a~different compliant password. Until that replacement succeeds, the~gallery does~not~establish the~normal viewer identity, issue \ui{Remember me}, or~allow viewer favourites/collections authority. The~ordinary forgotten-password reset can~also replace the~temporary credential through its~independently verified recovery flow. A~simultaneous administrator login in~the~same browser remains independent.

Existing viewer accounts are~listed on~the~Admin page with~their account state and~security controls. An~active viewer can~be~suspended with~\ui{Suspend} or~signed out everywhere; a~suspended viewer can~be~reactivated with~\ui{Restore}. Suspension blocks viewer authentication and~invalidates the~viewer's existing session, remember-me, reset, pending verification/email-change, and~prepared collection-share authority through the~central viewer lifecycle service. Restoration makes the~account active again but~does not~revive any credential or~session that existed before suspension, so~the~viewer must authenticate again normally. A~forced first-login password replacement remains required across suspend/restore. \ui{Sign out everywhere} leaves an~active account active while revoking its~viewer login authority on~all devices. These security controls remain available to~the~administrator while the~master feature is~enabled but~the~subordinate viewer frontend mode is~disabled. Turning the~master feature off hides the~viewer-management surface as~well. None of~these switches or~security controls removes favourites, private collections, collection items, photographs, galleries, Smart Galleries, gallery share links, or~administrator authentication.
...
The~public header shows \ui{Login} only while viewer accounts are~enabled. After sign-in it~shows \ui{Account}. The~account page shows the~verified email, links to~the~private \ui{Favourites} and~\ui{Collections} pages, \ui{Change password}, \ui{Change email}, \ui{Delete account}, and~a~viewer-only \ui{Log out} action. Logging out a~viewer or~changing viewer account authority does~not~log out an~administrator who happens to~be~authenticated in~the~same browser session.

\index{favourites}
A~signed-in viewer can~mark an~authorized photograph as~a~favourite from the~small heart control on~a~gallery card or~in~the~lightbox. The~same photograph is~kept synchronized between the~card and~lightbox, and~the~private \ui{Favourites} page lists saved photographs that the~viewer is~still allowed to~open. Removing a~favourite deletes only the~viewer's saved reference; it~does~not~delete or~modify the~photograph.
...
\index{viewer collections}
A~signed-in viewer can~also create private named \ui{Collections}. A~collection is~an~ordered list of~references to~existing photographs; it~is~not~a~gallery, does~not~copy photographs, and~does~not~change the~administrator-owned gallery hierarchy. From an~authorized gallery card, Smart Gallery card, or~the~private \ui{Favourites} page, the~viewer can~use \ui{Add to collection} and~choose one of~their own collections. The~private Collections area supports creating and~renaming a~collection, removing photographs, moving visible photographs up or~down, and~deleting the~collection itself.
...
Suspending the~collection owner revokes active collection shares, and~restoring the~account does~not~revive them. \ui{Sign out everywhere} only revokes viewer login authority and~does~not~revoke an~otherwise valid share. Deleting the~owner or~collection invalidates its~share without deleting source photographs. The~global Viewer Accounts feature switch also gates collection sharing and~remains disabled by~default.

Viewer \ui{Remember me} uses a~dedicated rotating viewer credential rather than~the~administrator persistent-login token. Restoring that credential signs in~only the~viewer and~does~not~count as~recent password reauthentication. Forgotten-password requests return a~generic response and~use a~two-step reset confirmation so~automated link scanners cannot reset a~password by~opening a~URL.

Sensitive account changes require recent viewer password proof. A~viewer restored only through \ui{Remember me} is~therefore asked to~confirm the~current viewer password before changing the~password, starting an~email-address change, or~deleting the~account. The~return path is~limited to~those account actions and~cannot redirect to~an~arbitrary page.

Changing the~viewer password uses the~same viewer password policy as~activation and~reset. A~successful change invalidates the~old viewer password authority, viewer sessions, and~viewer remember credentials according to~the~security lifecycle and~then asks the~viewer to~sign in~again. Saved favourites remain attached to~the~same viewer account. An~administrator signed in~through the~same browser session remains signed in.
...
The~Picture Manager is~a~logged-in administrator overlay on~the~public gallery view, not~a~visitor feature. An~administrator can~select photographs and~physical subgalleries, delete a~mixed selection, or~copy it~into a~new physical gallery below a~chosen parent. Photo-only selections can~also be~dragged onto a~visible child gallery, moved or~copied through the~searchable destination picker, shared through the~native device share sheet where supported, or~downloaded. Native sharing falls back to~the~selected-photo ZIP when the~browser cannot share the~prepared files. Smart Galleries are~excluded from~physical subtree operations. The~selection itself is~only browser state and~does not~grant permission. Every mutation/ZIP request rechecks the~normal logged-in account, CSRF token, source gallery, selected ownership, and~destination where applicable. Ordinary visitors do~not~receive these controls. Picture Manager is~intentionally independent from~the~Inline Administration capability: disabling Inline Administration removes direct public-page edit, add, delete, and~reorder affordances, while an~independently enabled Picture Manager can~still provide its~selection, deletion, move, copy, physical-gallery creation, and~selection-export workflows.
...
Version~0.106 added the~\codeword{maintenance\_jobs} table through an~ordinary migration. Run it~from the~Gallery administration page as~usual if~it~has~not~already been applied. It~requires the~installation's normal table-creation authority, but~does~not~require database triggers, stored routines, \codeword{SUPER}, a~global server-variable change, or~separate database-administrator intervention. Until the~migration is~available and~verified, the~Maintenance Center refuses to~start a~job instead of~attempting maintenance without durable checkpoints. Version~\version{} adds no~database migration.
...
\subsection{Gallery trash and~recovery}
\index{gallery!trash}
\index{gallery!restoration}
\index{trash!automatic purge}

Open \ui{Maintenance > Trash} to~review recoverable gallery deletions. Each active row shows the~original path, deletion time, gallery/photo counts, stored size, lifecycle status, and~automatic-purge deadline when that policy is~active. Problem entries remain visible for~inspection; PHP Gallery does~not~guess that an~ambiguous filesystem/database state is~safe to~destroy.
...
Migrations \filepath{202609070001_gallery_trash_bin.php} and~\filepath{202609070002_gallery_trash_state_machine.php} add the~durable operation records. When Trash is~enabled, deletion checks this schema before the~first filesystem move or~database-row deletion. Confirmed missing or~unknown storage stops recoverable deletion and~directs the~administrator to~apply/check migrations; it~does not~silently fall through to~permanent deletion. Scheduled Site Maintenance reconciles stale in-progress operations conservatively and~runs expired auto-purge work only when both Trash and~automatic purge are~enabled.

Trash payloads live in~protected persistent \filepath{data/gallery-trash/} storage rather than~the~cache or~live gallery tree. Deployment packages carry the~directory's access-policy file but~exclude stored trash contents. Include that runtime directory and~the~database in~backup and~restore planning; neither half alone is~a~complete recoverable entry.

\subsection{Maintenance Center}
\index{Maintenance Center}
\index{maintenance!central workflow}

Open \ui{Maintenance > Maintenance Center} for~a~guided \ui{Analyze}, \ui{Review}, \ui{Execute}, and~\ui{Verify} workflow across routine maintenance owners. Analyze is~read-only for~gallery, telemetry, log, Trash, cache, thumbnail, and~database content; it~writes only the~durable Maintenance Center checkpoint needed to~resume the~inspection. The~Review step shows required, default, and~optional tasks before any selected cleanup begins. Deep media verification is~optional and~is~not~selected automatically.

The~reviewed plan is~bound to~the~current application version, applied migration revision, Maintenance Center registry revision, and~plan content. If~one of~those inputs changes, execution refuses the~stale plan and~asks for~a~new analysis. The~server rechecks task dependencies, capability state, and~database-table eligibility at~execution time; browser checkboxes are~presentation input rather than~authorization.

Execution advances through bounded checkpoints, so~you can~reload or~close the~page and~resume the~same job later. Only one running or~paused central job owns the~durable mutation claim, and~two browser tabs cannot advance one job concurrently. Automatic Site Maintenance yields before its~first mutation while that claim is~held. Public read-only gallery requests remain available.

Pause and~Cancel are~cooperative: they take effect between bounded operations and~do~not~undo work already committed. A~database \codeword{ANALYZE TABLE} or~\codeword{OPTIMIZE TABLE} already running must~return before cancellation can~continue. The~center processes at~most one currently eligible table per~browser step, skips protected, unknown, oversized, or~newly ineligible tables, and~records an~uncertain atomic outcome instead of~blindly replaying a~table after a~lost response.

The~final report separates removed rows and~files, analyzed or~optimized tables, skips, warnings, and~errors. It~reports physical before/after storage only when inventory evidence supports that conclusion. The~generic workflow never applies migrations, performs schema repair, installs updates, deletes Admin-log archives, empties all Trash entries, or~regenerates media; use the~specialist pages for~those deliberate operations.

Download/cache cleanup advances through the~manifest cache, legacy download-artifact cache, and~generated-ZIP cache as~separate bounded steps. These caches are~independent optimizations: if~one is~unavailable on~the~current host, the~center preserves cleanup already completed, records a~safe warning, and~continues with the~other owners and~later tasks. The~warning can~identify the~operation and~a~stable error category, but~does~not~expose a~raw exception message, SQL, credential, private path, or~stack trace. After an~update changes this execution contract, an~older ready plan is~intentionally stale; run Analyze again before continuing.
...
\ui{Maintenance > Complete report} builds one self-contained HTML report covering storage, database structure and~size, gallery structure, image and~EXIF statistics, GPS overview, telemetry, operational logs, feature/settings state, and~runtime diagnostics. Image and~generated-media work is~processed in~bounded browser-driven batches so~the~report remains practical on~shared hosting. Transient timeout, rate-limit, and~server failures are~retried with~short increasing delays, while high-cardinality EXIF and~GPS summaries remain bounded to~control session-storage and~memory use on~large libraries. The~selected telemetry period affects only the~telemetry section; image statistics always cover the~complete image library. GPS place summaries use local approximate clustering and~offline reference data; probable simulator or~game screenshots are~excluded from~real-world place frequency where the~available evidence supports that classification.
...
The~same bounded model reports gallery edit-revision readiness and~unresolved image moves. A~missing or~unknown required schema is~an~action state and~stops the~affected mutation before target files or~rows change. Pending-move summaries contain only capped operation/gallery identities and~safe state categories; use the~documented command-line recovery workflow for~an~explicit operation rather than~expecting the~diagnostics page to~move files.
...
Picture Manager & Logged-in public-view administrative overlay with mixed photograph/physical-subgallery selection, recoverable deletion, chosen-parent physical-gallery creation with subtree copying and~rollback, photo-only move/copy/drag/share actions, native multi-file Web Share where supported, and~selected-photo ZIP fallback/download.\\
...
Admin gallery workspace & Visibility filter with hidden-row deselection, persistent per-branch collapse state, Collapse/Expand all, hierarchy reordering, visible-page public reorder tools, side-panel editors, and~contextual public-page edit/remove/visibility controls for~signed-in administrators.\\
...
Complete report & Self-contained HTML export covering storage, database, gallery/image/EXIF/GPS statistics, feature/settings state, telemetry, logs, and~runtime diagnostics, with bounded image/generated-media processing, transient-request retries, and~bounded high-cardinality summaries.\\
...
Maintenance Center & Durable Analyze--Review--Execute--Verify workflow, server-authorized task plan, bounded checkpoints, pause/resume/cancel, central mutation claim, one-table physical database steps, and~verified before/after report.\\
Scheduled site maintenance & Daily UTC window, optional request-triggered continuation, bounded image batch/time budget, CLI/web-cron support, run-one-slice action, interrupted-state reset, rotatable hidden web-cron token, thumbnail/metadata/cleanup/telemetry/log-archive tasks, and~yielding while the~Maintenance Center owns the~central mutation claim.\\
...
During the~short activation critical section, a~dependency-free early runtime gate prevents new ordinary requests from~loading a~partially replaced application. Such requests receive a~private, non-cacheable \codeword{503 Service Unavailable} response until activation completion is~durably recorded. The~authenticated update recovery/status path remains available, so~an~interrupted activation can~continue. Corrupt or~incomplete gate state fails closed instead of~presenting the~installation as~healthy.

The~same early runtime boundary converts uncaught and~catchable fatal PHP failures into bounded \codeword{500} responses when PHP can~still control the~response. Production hosting must keep \codeword{display\_errors} disabled; otherwise the~PHP runtime itself may~print warning or~fatal details before application-level handling can~format a~safe response.
...
Ordinary requests enter through \filepath{public/index.php}; the~repository-root \filepath{index.php} is~a~supported compatibility entry point for~other hosting layouts. The~public entry loads \filepath{app/early\_runtime.php} before \filepath{app/bootstrap.php}, then both layouts reach the~same normal request pipeline.
...
Keep a~verified database backup and~record the~migration filename, Gallery version, database-server version, bounded on-screen message, request reference, and~existing schema/ledger state. Retry through the~Gallery administration page so~the~normal idempotent compatibility path can~finish an~interrupted migration. Version~\version{} does~not~need triggers, stored routines, \codeword{SUPER}, \codeword{log\_bin\_trust\_function\_creators}, or~a~global database change. If~the~bounded error remains, inspect Runtime Diagnostics and~give the~request reference to~the~maintainer or~hosting provider; do~not~paste database credentials or~raw production data into a~support report.
...
\subsection{Validate a~recovery set}
\index{recovery!assurance}

Keep the~database export, original photographs, local configuration, relevant
\filepath{data/} storage, Trash contents, custom assets, and~exact application
version together as~one coordinated recovery set. A~provider snapshot or~a~bounded
maintenance window can~prevent writes from~splitting database and~file state.
Store an~off-host copy and~agree how much recent work may~be~lost and~how quickly
service must return.

The~CLI entry point \filepath{scripts/recovery.php} can~inventory that set
and~validate restored file samples and~manifest hashes in~a~separate location.
Follow \filepath{docs/RECOVERY_ASSURANCE.md} for~the~evidence format and~commands.
The~tool does~not~execute a~database dump, restore production, or~verify a~backup
provider. Its synthetic corruption drill proves the~checker detects fixture
damage, not~that your actual backup can~restore the~site.

Before opening a~restored installation, isolate email, scheduled jobs,
integrations, and~write access from~production destinations. Verify the~restored
database, relationships, Admin login, private-media denial, and~Trash restoration.
Record backup age, elapsed recovery time, and~the~operator's observations without
copying secrets into reports. Use \filepath{docs/RECOVERY_OFF_HOST.md} for~the~full
procedure. Updater file rollback still does~not~reverse database migrations.

\section{Maintainer verification and~release evidence}
\label{sec:release-evidence}
\index{release qualification}
\index{testing!disposable workflows}

Automated regression and~operational acceptance answer different questions.
The~central audit checks code, contracts, syntax, available browser fixtures,
and~release consistency. The~isolated workflow fixture adds real database,
HTTP, and~Chromium journeys using generated photographs and~accounts.
It~requires explicit disposable-only configuration and~must never use
the~installation's database or~media. Follow
\filepath{docs/GALLERY_WORKFLOWS.md} for~prerequisites; a~missing environment
is~a~coverage gap, not~proof of~a~working feature.

For~release preparation, follow \filepath{RELEASE.md}: finish source
and~documentation, rebuild and~inspect this PDF, generate the~manifest,
and~initialize the~qualification record. Run one central release audit,
not~a~quick/full/release sequence. With the~isolated prerequisites configured,
\filepath{scripts/gallery_workflow_mysql.php} accepts \codeword{--release}
to~provision its fixture around that same release audit.

The~qualification tool \filepath{scripts/release_qualification.php} records
the~exact source and~PDF fingerprint. Its \codeword{render} command produces
bounded page previews; rendering alone does~not~approve their appearance.
The~\codeword{record-audit} command accepts only the~matching central release
report, not~an~older full audit. A~changed source file or~rebuilt PDF selects
new pending evidence rather than silently reusing earlier approval.

Review the~title page, contents, index, changed pages, internal links,
and~representative browser workflows. Record what was~actually checked,
by~whom, and~with what result. Keep unresolved checks pending and~post-publication
updater smoke testing separate. A~green automated report is~not~permission
to~publish, and~the~tool never commits, tags, uploads, or~publishes
the~release. See \filepath{docs/RELEASE_QUALIFICATION.md} for~the~record commands.
