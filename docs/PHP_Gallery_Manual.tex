...
\newcommand{\manualdate}{16 August 2026}
...
\subsection{What is new in Version 0.90}
\index{Version 0.90}
...
Use \ui{Preview count}, save, and optionally publish. Images and files remain in their physical galleries, so metadata or rating changes alter membership immediately. The editorial rating is private and independent from visitor voting. Administrators may also use \ui{Save search as Smart Gallery} from public search.

Choose \ui{Unlisted} for URL-only access, \ui{Root gallery} to show the Smart Gallery on the homepage, or \ui{Subgallery placement} to keep it off the homepage while allowing physical galleries to list it. Each physical gallery editor contains a Smart Gallery checklist. The same virtual gallery may be selected beneath any number of physical galleries, and removing one placement does not disturb the others. The Smart Gallery editor's \ui{Used in physical galleries} section lists every attachment, links to each physical gallery editor, and provides an independent \ui{Hide from here} action. It must still be enabled and published before visitors see its card.
